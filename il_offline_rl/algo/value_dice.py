from il_offline_rl.model import AC_agent, Q_agent
...
class ValueDICE(BaseAlgo):
    def __init__(
        ...
        self.agent = AC_agent(input_dim, acs_dim, **agent_kwargs)
        self.agent.to(device)

        # init optimizer
        self.critic_optimizer = torch.optim.Adam(params=self.agent.get_critic_params(), lr=args.agent.nu_lr)
        ...
        self.replay_reg = args.agent.replay_regularization
        self.nu_reg = args.agent.nu_regularization
    ...
    def update(self):
        ...
        _, expert_next_act, _ = self.agent.get_action(expert_next_obs)
        _, rb_next_act, _ = self.agent.get_action(rb_next_obs)
        _, policy_initial_act, _ = self.agent.get_action(expert_initial_obs)
        ...
        expert_init_nu = self.agent.get_value(expert_init_obs_act)[0]
        expert_next_nu = self.agent.get_value(expert_next_obs_act)[0]
        expert_nu = self.agent.get_value(expert_obs_act)[0]

        rb_next_nu = self.agent.get_value(rb_next_obs_act)[0]
        rb_nu = self.agent.get_value(rb_obs_act)[0]
    ...
    def grad_penalty(self, expert_obs_act, rb_obs_act, expert_next_obs_act, rb_next_obs_act):
        ...
        total_q = self.agent.get_value(total_mix_obs_act)[0]
